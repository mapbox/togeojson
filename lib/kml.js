...
function getPlacemark(root, styleIndex, styleMapIndex, styleByHash) {
  ...
  if (iconStyle) {
    ...
    const color = iconstyles[0];
    const opacity = iconstyles[1];
    const scale = parseFloat(nodeVal(get1(iconStyle, "scale")));
    const heading = parseFloat(nodeVal(get1(iconStyle, "heading")));
    ...
    if (color) properties["icon-color"] = color;
    if (!isNaN(opacity)) properties["icon-opacity"] = opacity;
    if (!isNaN(scale)) properties["icon-size"] = scale;
    if (!isNaN(heading)) properties["icon-rotate"] = heading;
  }
  if (labelStyle) {
    const labelstyles = kmlColor(nodeVal(get1(labelStyle, "color")));
    const color = labelstyles[0];
    const opacity = labelstyles[1];
    const scale = parseFloat(nodeVal(get1(labelStyle, "scale")));
    if (color) properties["text-color"] = color;
    if (!isNaN(opacity)) properties["text-opacity"] = opacity;
    if (!isNaN(scale)) properties["text-size"] = scale;
  }
  if (lineStyle) {
