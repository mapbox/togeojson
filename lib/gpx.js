...
// const GARMIN_EXTENSIONS = ["hr", "cad", "atemp", "wtemp", "depth", "course", "bearing"];

/**
 * Handle line styling properties (stroke, stroke-opacity, stroke-width)
 */
...
/**
 * Parse single point attributes ("trkpt" or "rtept")
 */
...
/**
 * Extract points within a given segment (eg. "trkseg > trkpt", "rte > rtept")
 */
function getPoints(node, pointname) {
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    for (let j = 0; j < c.extensions.length; j++) {
      ...
      const propname = name.replace('gpxtpx:', ''); // strips garmin prefix
      if (!extensions[propname]) {
        extensions[propname] = Array(pts.length).fill(null);
      }
      extensions[propname][i] = val;
    }
  }
  return {
    line: line,
...
/**
 * Parse track segments (trk > trkseg)
 */
function getTrack(node) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const [name, val] of Object.entries(line.extensions)) {
      ...
      if (multi) {
        if (!props[name]) {
          props[name] = lines.map((line) => new Array(line.line.length).fill(null) );
        }
        props[name][i] = val;
      } else {
        props[name] = val;
